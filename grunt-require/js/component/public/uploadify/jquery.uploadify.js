		...
		init : function(options, swfUploadOptions) {
			...
			return this.each(function() {
				...
				if (flashInstalled) {
				    ...
				    swfUploadSettings.button_image_url="";
				...
				if (args[0]) {
					...
					if (args[0] == '*') {
						...
						$('#' + settings.queueID).find('.uploadify-queue-item').each(function() {
							...
							$(this).delay(settings.deply + 100 * delay).fadeOut(settings.speed, function () {
								$(this).remove();
							});
						});
						...
						for (var n = 0; n < args.length; n++) {
							...
							$('#' + args[n]).delay(settings.deply + 100 * n).fadeOut(settings.speed, function () {
								$(this).remove();
							});
						}
					}
					...
					$item.delay(settings.deply).fadeOut(settings.speed, function () {
						$(this).remove();
					});
				}
			});
						...
						case SWFUpload.FILE_STATUS.COMPLETE:
							setTimeout(function() { 
								if ($('#' + file.id)) {
									...
									$('#' + file.id).fadeOut(settings.speed, function () {
										$(this).remove();
									});
								}
							}, settings.removeTimeout * settings.deply);
							break;
